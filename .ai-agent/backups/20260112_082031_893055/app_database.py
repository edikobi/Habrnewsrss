...
class User(Base):
    ...
    def ensure_settings(self, session: Session) -> 'UserSettings':
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error ensuring settings for user {self.id}: {e}")
            raise
    ...
    try:
        ...
        if engine.url.drivername == 'sqlite':
            try:
                ...
                # Получить информацию о столбцах таблицы content_items
                cursor.execute("PRAGMA table_info(content_items)")
                existing_columns = [col[1] for col in cursor.fetchall()]

                # Определить все столбцы, которые должны быть в таблице согласно модели
                required_columns = {
                    'full_text': 'TEXT',
                    'content_type': 'TEXT',
                    'platform': 'TEXT', 
                ...
                for column_name, column_type in required_columns.items():
                    if column_name not in existing_columns:
                        try:
                            cursor.execute(f"ALTER TABLE content_items ADD COLUMN {column_name} {column_type}")
                            print(f"Добавлен столбец {column_name} в таблицу content_items")
                        except Exception as e:
                ...
                print(f"Предупреждение: ошибка при выполнении миграции SQLite: {e}")
