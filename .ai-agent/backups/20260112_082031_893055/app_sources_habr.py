...
class HabrSource(ContentSource):
    ...
    def fetch_content(self, keywords: List[str], max_results: int = 30) -> List[ContentItem]:
        ...
        try:
            ...
            for entry in feed.entries:
                ...
                full_text = self._fetch_full_text(entry.link)
            ...
            for item in items:
                ...
                if not exists:
                    ...
                    if not item.full_text:
                        item.full_text = self._fetch_full_text(item.url)
    ...
    def _fetch_full_text(self, url: str, timeout: int = 10) -> str:
        """Загружает полный текст статьи Habr по URL, извлекая содержимое поста."""
        headers = {
        ...
        try:
            ...
            # Поиск основного контейнера текста
            container = (
            ...
            import logging
            logging.getLogger(__name__).warning(f"Error fetching full text from {url}: {e}")
            return ""