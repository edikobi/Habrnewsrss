...
class EmailSender:
    ...
    def send_digest(self, user: User, items: List[ContentItem]) -> bool:
        """Send daily digest email to user with recommended content items."""
        if not items:
            return False
            
        try:
            message = self._create_digest_message(user, items)
            if self._connect():
    ...
    def _create_digest_message(self, user: User, items: List[ContentItem]) -> MIMEMultipart:
