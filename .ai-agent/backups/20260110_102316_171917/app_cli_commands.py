...
@click.option('--interests', help='Интересы через запятую')
...
@cli.command()
@click.option('--user-id', type=int, required=True, help='ID пользователя')
@click.option('--max-items', default=10, help='Максимальное количество элементов')
@click.option('--test', is_flag=True, help='Тестовый режим (не отправляет email)')
def send_digest(user_id: int, max_items: int, test: bool) -> None:
    ...
    if test:
        ...
        success = email_sender.send_digest(user, digest)
        if success:
