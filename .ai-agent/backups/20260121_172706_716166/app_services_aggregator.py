...
class ContentAggregator:
    ...
    def aggregate_by_keywords(self, keywords: List[str], max_per_source: int = 20) -> List[ContentItem]:
        ...
        logger.info(f"Агрегация контента по ключевым словам: {keywords}")
        
        # Нормализовать ключевые слова
        keywords = [kw.strip().lower() for kw in keywords if kw and kw.strip()]
        if not keywords:
            logger.warning("Не предоставлены валидные ключевые слова для агрегации")
            return []
        
        logger.info(f"Нормализованные ключевые слова: {keywords}")
        logger.info(f"Доступные источники: {[s.__class__.__name__ for s in self.sources]}")
        ...
        for source in self.sources:
            ...
            if source_name == 'HabrSource' and not settings.habr_enabled:
                logger.info(f"Источник {source_name} отключен в настройках, пропускаем")
                continue
            elif source_name == 'YouTubeSource' and (not settings.youtube_enabled or not settings.youtube_api_key):
                logger.info(f"Источник {source_name} отключен или нет API ключа, пропускаем")
                continue
            elif source_name == 'CourseraSource' and (not settings.coursera_enabled or not settings.coursera_api_key):
                logger.info(f"Источник {source_name} отключен или нет API ключа, пропускаем")
                continue
            
            active_sources += 1
            logger.info(f"Запрос к источнику {source_name} с лимитом {max_per_source}")
            
            # Увеличиваем лимит для Habr, так как это основной источник статей
            actual_max = max_per_source * 2 if source_name == 'HabrSource' else max_per_source
            
            try:
                items = source.fetch_content(keywords, actual_max)
                logger.info(f"Источник {source_name} вернул {len(items)} статей")
                results.extend(items)
            except Exception as e:
                logger.error(f"Ошибка при запросе к источнику {source_name}: {e}")
        
        if active_sources == 0:
            logger.warning("Нет активных источников для агрегации контента")
            return []
        ...
        logger.info(f"Итого уникальных статей после дедупликации: {len(unique_results)}")
    ...
    def update_content_for_user(self, user_id: int) -> int:
        ...
        user = self.db_session.query(User).get(user_id)
        if not user:
            logger.warning(f"User {user_id} not found for content update")
            return 0

        keywords = [interest.tag_name for interest in user.interests]
        if not keywords:
            logger.info(f"User {user_id} has no interests configured")
            return 0

        # Логирование деталей интересов пользователя
        logger.info(f"Интересы пользователя {user_id} ({user.username}): {keywords}")
        
        # Проверка на русскоязычные ключевые слова
        russian_keywords = [kw for kw in keywords if any(cyr in kw for cyr in 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя')]
        if russian_keywords:
            logger.info(f"Русскоязычные ключевые слова: {russian_keywords}")

        logger.info(f"Updating content for user {user_id} with interests: {keywords}")
        
        # Увеличиваем лимит для получения большего количества статей
        items = self.aggregate_by_keywords(keywords, max_per_source=150)
        logger.info(f"Found {len(items)} total articles from all sources")
        ...
        logger.info(f"After deduplication: {len(filtered_items)} new articles")
        count = self.save_content_items(filtered_items)
        logger.info(f"Saved {count} new articles for user {user_id}")

        # Резервный поиск: если новых статей нет, ищем более старые, но еще не загруженные
        if count == 0:
            try:
                logger.info(f"No new articles found for user {user_id}. Starting backup search...")
                ...
                logger.info(f"Backup search found {len(backup_items)} articles")
                ...
                if backup_filtered:
                    ...
                    count += backup_count
                    logger.info(f"Loaded {backup_count} older articles from backup search for user {user_id}")
            except Exception as e:
                logger.error(f"Backup search error for user {user_id}: {e}")
    ...
    def save_selected_items(self, items: List[ContentItem], source_name: str = "habr", user_id: Optional[int] = None) -> List[ContentItem]:
        ...
        try:
            ...
            if hasattr(source, 'save_selected_items'):
                ...
                # Update user interests based on selected content
                if user_id is not None:
                    try:
                        items_to_process = saved if saved else existing_items
                        if items_to_process:
                            self.add_user_interests_from_content(user_id, items_to_process)
                    except Exception as e:
                        logger.error(f"Error updating interests during save: {e}")

                return saved if saved else existing_items
    ...
    def add_user_interests_from_content(self, user_id: int, content_items: List[ContentItem]) -> None:
        """Добавить или обновить интересы пользователя на основе тегов выбранных статей.
        Для каждого тега каждой статьи:
        - Если у пользователя ещё нет интереса с этим тегом, создаётся UserInterest with priority=5.
        - Если интерес уже существует, его priority увеличивается на 1 (максимум 10).
        """
        if not user_id or user_id <= 0 or not content_items:
            return
        ...
        try:
            for item in content_items:
                for tag in item.tags:
                    ...
                    interest = self.db_session.query(UserInterest).filter_by(
                        user_id=user_id, 
                        tag_name=tag_name
                    ).first()

                    if not interest:
                        new_interest = UserInterest(user_id=user_id, tag_name=tag_name, priority=5)
                        self.db_session.add(new_interest)
                    else:
                        interest.priority = min(interest.priority + 1, 10)

            self.db_session.commit()
        except Exception as e:
            logger.error(f"Error updating user interests from content: {e}")
            self.db_session.rollback()
