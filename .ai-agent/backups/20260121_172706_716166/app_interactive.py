...
class InteractiveMenu:
    ...
    def add_new_user_interactive(self) -> None:
        ...
        if not email or "@" not in email:
            print(Fore.RED + "Некоррексный email.")
            return
    ...
    def search_content(self) -> None:
        """Интерактивный поиск с выбором источника (БД или Habr), сохранением запроса и обновлением интересов."""
        print(Fore.CYAN + "\n" + "═"*40)
        print(Fore.CYAN + "ПОИСК КОНТЕНТА")
        print(Fore.CYAN + "─"*40)
        print(f"{Fore.YELLOW}1.{Fore.WHITE} Поиск в базе данных")
        print(f"{Fore.YELLOW}2.{Fore.WHITE} Поиск в интернете (Habr)")

        source_choice = input(Fore.YELLOW + "\nВыберите источник поиска (1-2): ").strip()

        keywords_input = input(Fore.YELLOW + "Введите ключевые слова через запятую: ")
        keywords = [k.strip() for k in keywords_input.split(",") if k.strip()]

        if not keywords:
            print(Fore.RED + "Ключевые слова не введены.")
            return
        ...
        if source_choice == "1":
            ...
            if not results:
                print(Fore.YELLOW + "В базе данных ничего не найдено.")
                return
            print(Fore.GREEN + f"\nНайдено {len(results)} статей в БД:")

        elif source_choice == "2":
            # Поиск в интернете (Habr)
            print(Fore.CYAN + f"Поиск на Habr по ключевым словам: {', '.join(keywords)}...")
            results = self.aggregator.search_live(keywords, source_name="habr", max_results=30)
            if not results:
                print(Fore.YELLOW + "На Habr ничего не найдено.")
                return
            print(Fore.GREEN + f"\nНайдено {len(results)} статей на Habr:")

        else:
            print(Fore.RED + "Некорректный выбор источника.")
            return

        # Показать результаты поиска с нумерацией
        for i, item in enumerate(results, 1):
            if source_choice == "1":
                # Для результатов из БД показать дату добавления
                add_date = item.added_at.strftime('%d.%m.%y %H:%M') if item.added_at else "Неизвестно"
                print(f"{Fore.YELLOW}{i}.{Fore.WHITE} {item.title}")
                print(f"   Платформа: {Fore.CYAN}{item.platform}{Fore.WHITE} | Добавлено: {add_date}")
            else:
                # Для результатов из интернета показать только заголовок и URL
                print(f"{Fore.YELLOW}{i}.{Fore.WHITE} {item.title}")
                print(f"   URL: {Fore.BLUE}{item.url}")

        try:
            input_text = input(Fore.YELLOW + "\nВведите номера статей через запятую для сохранения в БД (или 0 для выхода): ")
            if input_text.strip() == "0":
                return

            selected_indices = [int(idx.strip()) for idx in input_text.split(",") if idx.strip().isdigit()]
            selected_items = [results[i-1] for i in selected_indices if 1 <= i <= len(results)]

            if not selected_items:
                print(Fore.YELLOW + "Не выбрано ни одной статьи.")
                return

            if source_choice == "1":
                # Для статей из БД - только обновить интересы пользователя
                if self.current_user_id and selected_items:
                    self.aggregator.add_user_interests_from_content(self.current_user_id, selected_items)
                    print(Fore.GREEN + f"✓ Интересы обновлены на основе {len(selected_items)} выбранных статей")
            else:
                # Для статей из интернета - сохранить в БД и обновить интересы
                saved_items = self.aggregator.save_selected_items(
                    selected_items, 
                    source_name="habr", 
                    user_id=self.current_user_id
                )
                ...
                if saved_items:
                    msg = f"✓ Сохранено {len(saved_items)} статей в БД"
                    if self.current_user_id:
                        msg += ", интересы обновлены"
                    print(Fore.GREEN + msg)

                    # Предложить просмотреть первую сохраненную статью
                    if saved_items:
                        view_choice = input(Fore.YELLOW + "Просмотреть первую сохранённую статью? (y/n): ")
                        if view_choice.lower() == 'y':
                            self.view_content_details(saved_items[0].id)
                else:
                    print(Fore.YELLOW + "Статьи уже существуют в БД или не удалось сохранить.")

        except ValueError:
            print(Fore.RED + "Ошибка: введите корректные номера через запятую.")
        except Exception as e:
    ...
    def get_recommendations_interactive(self) -> None:
        ...
        try:
            if choice == "1":
                ...
                choice_input = input(Fore.YELLOW + "\nВведите номера статей через запятую для обновления интересов (или 0 для пропуска): ")
                if choice_input.strip() != "0":
                    try:
                        indices = [int(idx.strip()) for idx in choice_input.split(",") if idx.strip().isdigit()]
                        selected_items = [recs[i-1] for i in indices if 1 <= i <= len(recs)]
                        if selected_items and self.current_user_id:
                            self.aggregator.add_user_interests_from_content(self.current_user_id, selected_items)
                            print(Fore.GREEN + f"✓ Интересы обновлены на основе {len(selected_items)} выбранных статей")
                    except ValueError:
                        print(Fore.RED + "Некорректный ввод номеров.")
                ...
                if choice_input.strip() != "0":
                    try:
                        indices = [int(idx.strip()) for idx in choice_input.split(",") if idx.strip().isdigit()]
                        selected_items = [live_items[i-1] for i in indices if 1 <= i <= len(live_items)]
                        if selected_items:
                            saved_items = self.aggregator.save_selected_items(selected_items, source_name="habr", user_id=self.current_user_id)
                            print(Fore.GREEN + f"✓ Сохранено {len(saved_items)} статей" + 
                                  (f", интересы обновлены" if self.current_user_id else ""))
