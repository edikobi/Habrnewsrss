...
class InteractiveMenu:
    ...
    def view_content_details(self, content_id: int) -> None:
        ...
        try:
            ...
            # Display full text with bidirectional page navigation
            if item.full_text and item.full_text.strip():
                ...
                try:
                    chunk_size = 3500  # Optimal for terminal display (~60 lines)
                    ...
                    while True:
                        ...
                        chunk = item.full_text[start:end]
                        print(Fore.WHITE + chunk)

                        # Show page indicator and navigation
                        print(Fore.CYAN + f"\n[Страница {current_page + 1} из {total_pages}]")
                        print(Fore.YELLOW + "n - следующая | p - предыдущая | число - перейти к странице | q - выход")
                        ...
                        if page_choice == 'n':
                            if current_page < total_pages - 1:
                                ...
                                print(Fore.YELLOW + "Вы уже на последней странице.")
                        elif page_choice == 'p':
                            if current_page > 0:
                                current_page -= 1
                            else:
                                print(Fore.YELLOW + "Вы уже на первой странице.")
                        elif page_choice == 'q':
                            break
                        elif page_choice.isdigit():
                            try:
                                page_num = int(page_choice)
                                if 1 <= page_num <= total_pages:
                                    current_page = page_num - 1
                                else:
                                    print(Fore.RED + f"Введите число от 1 до {total_pages}.")
                            except ValueError:
                                print(Fore.RED + "Неверный формат номера страницы.")
                        else:
                            print(Fore.RED + "Неверная команда. Используйте n/p/число/q.")
                except KeyboardInterrupt:
            ...
            if self.current_user_id:
                ...
                print(f"{Fore.YELLOW}4.{Fore.WHITE} Назад к списку")
