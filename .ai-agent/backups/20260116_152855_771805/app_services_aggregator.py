...
class ContentAggregator:
    ...
    def update_content_for_user(self, user_id: int) -> int:
        ...
        # Note: last_content_update timestamp is now managed by check_and_update_content_for_all_users
        # to ensure atomic commits
        return count
    ...
    def _should_update_content(self, settings_obj: UserSettings, now: datetime) -> bool:
        """Определяет, нужно ли выполнить обновление контента для пользователя.
           ...
           1. Пользователь включил автообновление и дайджесты (auto_update_content и digest_enabled)
           2. Либо last_content_update отсутствует (никогда не обновлялось)
           3. Либо текущее время превысило ближайшее запланированное время обновления.
           Запланированное время вычисляется как ближайший момент после last_content_update,
           когда час равен digest_hour, с учётом интервала content_update_interval_hours."""
        try:
