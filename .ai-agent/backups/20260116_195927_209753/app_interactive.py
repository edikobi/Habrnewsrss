...
class InteractiveMenu:
    ...
    def search_content(self) -> None:
        """Интерактивный поиск с сохранением запроса пользователя и возможностью сохранения статей."""
        ...
        try:
            ...
            if selected_items:
                ...
                for platform, items in items_by_platform.items():
                    saved = self.aggregator.save_selected_items(items, source_name=platform, user_id=self.current_user_id)
                    total_saved += len(saved)

                msg = f"✓ Сохранено {total_saved} статей."
                if self.current_user_id:
                    print(Fore.GREEN + msg + " Интересы обновлены.")
                else:
                    print(Fore.GREEN + msg)
                    print(Fore.YELLOW + "⚠ Статьи сохранены без привязки к пользователю (пользователь не выбран).")
        except ValueError:
