...
class InteractiveMenu:
    ...
    def run(self) -> None:
        ...
        # Try to restore last user from settings
        try:
    ...
    def view_content(self) -> None:
        ...
        if user_id:
            try:
                ...
                if mark_choice.isdigit() and 0 < int(mark_choice) <= len(digest):
                    item = digest[int(mark_choice)-1]
                    ...
                    if fav_q == 'y':
                        fav = FavoriteContent(user_id=user_id, content_id=item.id)
                        self.session.add(fav)
                    
                    self.session.commit()
                    print(Fore.GREEN + "Обновлено!")
    ...
    def search_content(self) -> None:
        ...
        if mode == "internet":
            results = self.aggregator.search_live(keywords, source_filter)
            if not results:
            ...
            if save_choice and save_choice != "0":
                try:
                    ...
                    self.aggregator.save_selected_items(selected, source_filter)
                    ...
                    if self.current_user_id:
                        for item in selected:
                            ...
                            if db_item:
                                ...
                                self.session.add(fav)
                        self.session.commit()
                    print(Fore.GREEN + f"Обработано {len(selected)} элементов.")
                except Exception as e:
    ...
    def configure_email_settings(self) -> None:
        ...
        while True:
            ...
            if not settings:
                settings = UserSettings(user_id=user.id, email_digest=user.email)
                self.session.add(settings)
                self.session.commit()

            print(Fore.CYAN + "\nНАСТРОЙКИ EMAIL")
            print(f"1. Email: {settings.email_digest}")
            print(f"2. Час рассылки: {settings.digest_hour}:00")
            print(f"3. Статус: {'Включена' if settings.digest_enabled else 'Выключена'}")
            print(f"4. Отправлять пропущенные: {'Да' if settings.missed_digest_send else 'Нет'}")
            print("5. Протестировать (отправить сейчас)")
            print("6. Назад")
            ...
            try:
                if choice == "1":
                    ...
                    status = "включена" if settings.digest_enabled else "выключена"
                    print(Fore.GREEN + f"Рассылка {status}.")
                elif choice == "4":
                    settings.missed_digest_send = not settings.missed_digest_send
                    status = "включена" if settings.missed_digest_send else "выключена"
    ...
    def send_test_digest(self) -> None:
        ...
        items = self.aggregator.get_daily_digest(self.current_user_id, max_items=5)
        if not items:
            print(Fore.YELLOW + "Нет контента для подборки.")
            return
