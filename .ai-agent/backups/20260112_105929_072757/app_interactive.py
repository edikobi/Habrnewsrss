...
class InteractiveMenu:
    ...
    def view_content_details(self, content_id: int) -> None:
        ...
        try:
            ...
            # Display full text if available, otherwise description
            if item.full_text and item.full_text.strip():
                print(Fore.WHITE + "\nПОЛНЫЙ ТЕКСТ СТАТЬИ:")
                # Limit display to 3000 characters to avoid terminal overflow
                if len(item.full_text) > 3000:
                    print(Fore.WHITE + item.full_text[:3000])
                    print(Fore.YELLOW + "\n...\n[Текст обрезан. Полная версия доступна по ссылке]")
                else:
                    print(Fore.WHITE + item.full_text)
            else:
                print(Fore.WHITE + "\nОПИСАНИЕ:")
